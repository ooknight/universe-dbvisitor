 ...
 */
package io.github.ooknight.universe.dbvisitor.guice;
...
import io.github.ooknight.universe.dbvisitor.transaction.TransactionManager;
...
import io.github.ooknight.universe.dbvisitor.guice.provider.JdbcTemplateProvider;
import io.github.ooknight.universe.dbvisitor.guice.provider.LambdaTemplateProvider;
import io.github.ooknight.universe.dbvisitor.guice.provider.TransactionManagerProvider;
import io.github.ooknight.universe.dbvisitor.guice.provider.TransactionTemplateProvider;
import io.github.ooknight.universe.dbvisitor.jdbc.JdbcOperations;
import io.github.ooknight.universe.dbvisitor.jdbc.core.JdbcTemplate;
import io.github.ooknight.universe.dbvisitor.lambda.LambdaOperations;
import io.github.ooknight.universe.dbvisitor.lambda.LambdaTemplate;
import io.github.ooknight.universe.dbvisitor.mapping.Options;
import io.github.ooknight.universe.dbvisitor.session.Configuration;
import io.github.ooknight.universe.dbvisitor.session.Session;
import io.github.ooknight.universe.dbvisitor.transaction.support.TransactionHelper;
import org.aopalliance.intercept.MethodInterceptor;
import org.aopalliance.intercept.MethodInvocation;
import static io.github.ooknight.universe.dbvisitor.guice.ConfigKeys.*;
...
public class DbVisitorModule implements com.google.inject.Module {
    ...
    private void bindTrans(String dbName, Binder binder, Provider<DataSource> dsProvider) {
        Provider<io.github.ooknight.universe.dbvisitor.transaction.TransactionManager> managerProvider = new TransactionManagerProvider(dsProvider);
        Provider<io.github.ooknight.universe.dbvisitor.transaction.TransactionTemplate> templateProvider = new TransactionTemplateProvider(dsProvider);

        if (StringUtils.isBlank(dbName)) {
            binder.bind(io.github.ooknight.universe.dbvisitor.transaction.TransactionManager.class).toProvider(managerProvider);
            binder.bind(io.github.ooknight.universe.dbvisitor.transaction.TransactionTemplate.class).toProvider(templateProvider);
        } else {
            binder.bind(io.github.ooknight.universe.dbvisitor.transaction.TransactionManager.class).annotatedWith(Names.named(dbName)).toProvider(managerProvider);
            binder.bind(io.github.ooknight.universe.dbvisitor.transaction.TransactionTemplate.class).annotatedWith(Names.named(dbName)).toProvider(templateProvider);
        }

        TranInterceptor tranInter = new TranInterceptor(dsProvider);
        binder.bindInterceptor(new ClassAnnotationOf(io.github.ooknight.universe.dbvisitor.transaction.Transactional.class), new MethodAnnotationOf(io.github.ooknight.universe.dbvisitor.transaction.Transactional.class), tranInter);
    }
    ...
    private static class TranInterceptor implements MethodInterceptor {
        ...
        /* 是否不需要回滚:true表示不要回滚 */
        private boolean testNoRollBackFor(io.github.ooknight.universe.dbvisitor.transaction.Transactional tranAnno, Throwable e) {
            //1.test Class
        ...
        public final Object invoke(final MethodInvocation invocation) throws Throwable {
            ...
            io.github.ooknight.universe.dbvisitor.transaction.Transactional tranInfo = tranAnnotation(targetMethod);
            if (tranInfo == null) {
                return invocation.proceed();
            }
            //0.准备事务环境
            TransactionManager manager = TransactionHelper.txManager(this.dataSource.get());
            io.github.ooknight.universe.dbvisitor.transaction.Propagation behavior = tranInfo.propagation();
            io.github.ooknight.universe.dbvisitor.transaction.Isolation level = tranInfo.isolation();
            io.github.ooknight.universe.dbvisitor.transaction.TransactionStatus tranStatus = manager.begin(behavior, level);
            //1.只读事务
        ...
        private io.github.ooknight.universe.dbvisitor.transaction.Transactional tranAnnotation(Method targetMethod) {
            io.github.ooknight.universe.dbvisitor.transaction.Transactional tran = targetMethod.getAnnotation(io.github.ooknight.universe.dbvisitor.transaction.Transactional.class);
            if (tran == null) {
                Class<?> declaringClass = targetMethod.getDeclaringClass();
                tran = declaringClass.getAnnotation(io.github.ooknight.universe.dbvisitor.transaction.Transactional.class);
            }
            return tran;
        }
