 ...
 */
package io.github.ooknight.universe.dbvisitor.jdbc.core;
import java.io.*;
...
import io.github.ooknight.universe.dbvisitor.dynamic.segment.TextSqlSegment;
import io.github.ooknight.universe.dbvisitor.jdbc.*;
import io.github.ooknight.universe.dbvisitor.jdbc.extractor.*;
...
import io.github.ooknight.universe.dbvisitor.dynamic.args.ArraySqlArgSource;
import io.github.ooknight.universe.dbvisitor.dynamic.args.BeanSqlArgSource;
import io.github.ooknight.universe.dbvisitor.dynamic.args.MapSqlArgSource;
import io.github.ooknight.universe.dbvisitor.dynamic.args.SqlArgDisposer;
...
import io.github.ooknight.universe.dbvisitor.error.UncategorizedSQLException;
import io.github.ooknight.universe.dbvisitor.jdbc.*;
import io.github.ooknight.universe.dbvisitor.jdbc.extractor.*;
import io.github.ooknight.universe.dbvisitor.jdbc.mapper.BeanMappingRowMapper;
import io.github.ooknight.universe.dbvisitor.jdbc.mapper.ColumnMapRowMapper;
import io.github.ooknight.universe.dbvisitor.jdbc.mapper.SingleColumnRowMapper;
...
public class JdbcTemplate extends JdbcConnection implements JdbcOperations {
    ...
    protected <T> T executeCreator(final PreparedStatementCreator psc, final PreparedStatementCallback<T> action) {
        //if (logger.isDebugEnabled()) {
    ...
    @Override
    public <T> T executeCreator(PreparedStatementCreator psc, ResultSetExtractor<T> rse) {
    ...
    @Override
    public void executeCreator(PreparedStatementCreator psc, RowCallbackHandler rch) {
        this.executeCreator(psc, new RowCallbackHandlerResultSetExtractor(rch));
    }

    @Override
    public <T> List<T> executeCreator(PreparedStatementCreator psc, RowMapper<T> rowMapper) {
        List<T> result = this.executeCreator(psc, new RowMapperResultSetExtractor<>(rowMapper));
        return result == null ? Collections.emptyList() : result;
    }

    @Override
    public <T> T executeCreator(CallableStatementCreator csc, CallableStatementCallback<T> action) {
        //if (logger.isDebugEnabled()) {
    ...
    @Override
    public Map<String, Object> call(String callString) {
        return this.call(callString, ArrayUtils.EMPTY_OBJECT_ARRAY);
    }

    @Override
    public Map<String, Object> call(String callString, Object args) {
    ...
    @Override
    public <T> T call(String callString, CallableStatementSetter args, CallableStatementCallback<T> callback) {
        return this.executeCreator(this.getCallableStatementCreator(callString, args), callback);
    }

    @Override
    public Map<String, Object> multipleExecute(final String sql) {
        return this.multipleExecute(sql, ArrayUtils.EMPTY_OBJECT_ARRAY);
    }

    @Override
    public Map<String, Object> multipleExecute(final String sql, final Object args) {
    ...
    @Override
    public Map<String, Object> multipleExecute(final String sql, final PreparedStatementSetter args) {
    ...
    @Override
    public <T> T query(final String sql, final Object args, final ResultSetExtractor<T> rse) {
        if (args instanceof PreparedStatementSetter) {
    ...
    @Override
    public <T> T query(final String sql, final PreparedStatementSetter args, final ResultSetExtractor<T> rse) {
    ...
    @Override
    public void query(final String sql, final Object args, final RowCallbackHandler rch) {
        if (args instanceof PreparedStatementSetter) {
    ...
    @Override
    public void query(final String sql, final PreparedStatementSetter args, final RowCallbackHandler rch) {
        PreparedStatementCreator psc = this.getPreparedStatementCreator(sql, args);
        this.executeCreator(psc, new RowCallbackHandlerResultSetExtractor(rch));
    }
    ...
    @Override
    public <T> List<T> queryForList(final String sql, final Object args, final RowMapper<T> rowMapper) {
        if (args instanceof PreparedStatementSetter) {
    ...
    @Override
    public <T> List<T> queryForList(final String sql, final PreparedStatementSetter args, final RowMapper<T> rowMapper) {
        PreparedStatementCreator psc = this.getPreparedStatementCreator(sql, args);
        return this.executeCreator(psc, new RowMapperResultSetExtractor<>(rowMapper));
    }

    @Override
    public <T> List<T> queryForList(final String sql, final Class<T> elementType) {
        return this.queryForList(sql, ArrayUtils.EMPTY_OBJECT_ARRAY, elementType);
    }

    @Override
    public <T> List<T> queryForList(final String sql, final Object args, final Class<T> elementType) {
        if (args instanceof PreparedStatementSetter) {
    ...
    @Override
    public <T> List<T> queryForList(final String sql, final PreparedStatementSetter args, final Class<T> elementType) {
        PreparedStatementCreator psc = this.getPreparedStatementCreator(sql, args);
        return this.executeCreator(psc, this.createBeanResultSetExtractor(elementType));
    }
    ...
    @Override
    public List<Map<String, Object>> queryForList(final String sql, final PreparedStatementSetter args) {
        PreparedStatementCreator psc = this.getPreparedStatementCreator(sql, args);
        return this.executeCreator(psc, new RowMapperResultSetExtractor<>(this.createMapRowMapper()));
    }
    ...
    @Override
    public <T> T queryForObject(final String sql, final Object args, final RowMapper<T> rowMapper) {
        if (args instanceof PreparedStatementSetter) {
    ...
    @Override
    public <T> T queryForObject(final String sql, final PreparedStatementSetter args, final RowMapper<T> rowMapper) {
        PreparedStatementCreator psc = this.getPreparedStatementCreator(sql, args);
        List<T> result = this.executeCreator(psc, new RowMapperResultSetExtractor<>(rowMapper, 1));
        return requiredSingleResult(result);
    }
    ...
    @Override
    public <T> T queryForObject(final String sql, final Class<T> requiredType) {
        return this.queryForObject(sql, ArrayUtils.EMPTY_OBJECT_ARRAY, this.createBeanRowMapper(requiredType));
    }

    @Override
    public <T> T queryForObject(final String sql, final Object args, final Class<T> requiredType) {
        return this.queryForObject(sql, args, this.createBeanRowMapper(requiredType));
    }

    @Override
    public <T> T queryForObject(final String sql, final PreparedStatementSetter args, final Class<T> requiredType) {
        return this.queryForObject(sql, args, this.createBeanRowMapper(requiredType));
    }
    ...
    @Override
    public Map<String, Object> queryForMap(final String sql, final PreparedStatementSetter args) {
        return this.queryForObject(sql, args, this.createMapRowMapper());
    }

    @Override
    public <K, V> Map<K, V> queryForPairs(String sql, Class<K> pairKey, Class<V> pairValue) {
        return this.query(sql, ArrayUtils.EMPTY_OBJECT_ARRAY, this.createPairsResultSetExtractor(pairKey, pairValue));
    }

    @Override
    public <K, V> Map<K, V> queryForPairs(String sql, Class<K> pairKey, Class<V> pairValue, Object args) {
        return this.query(sql, args, this.createPairsResultSetExtractor(pairKey, pairValue));
    }

    @Override
    public <K, V> Map<K, V> queryForPairs(String sql, Class<K> pairKey, Class<V> pairValue, PreparedStatementSetter args) {
        return this.query(sql, args, this.createPairsResultSetExtractor(pairKey, pairValue));
    }
    ...
    @Override
    public Long queryForLong(final String sql, final PreparedStatementSetter args) {
        Number number = this.queryForObject(sql, args, this.createSingleColumnRowMapper(long.class));
        return number != null ? number.longValue() : null;
    }
    ...
    @Override
    public Integer queryForInt(final String sql, final PreparedStatementSetter args) {
        Number number = this.queryForObject(sql, args, this.createSingleColumnRowMapper(int.class));
        return number != null ? number.intValue() : null;
    }
    ...
    @Override
    public String queryForString(final String sql, final Object args) {
        return this.queryForObject(sql, args, this.createSingleColumnRowMapper(String.class));
    }

    @Override
    public String queryForString(final String sql, final PreparedStatementSetter args) {
        return this.queryForObject(sql, args, this.createSingleColumnRowMapper(String.class));
    }
    ...
    @Override
    public int executeUpdate(final String sql, final PreparedStatementSetter args) {
        PreparedStatementCreator psc = this.getPreparedStatementCreator(sql, args);
        return this.executeCreator(psc, (PreparedStatementCallback<Integer>) PreparedStatement::executeUpdate);
    }
    ...
    @Override
    public int[] executeBatch(String sql, Object[] batchArgs) {
        ...
        if (usingPreparedStatement && usingArgSource) {
            throw new RuntimeSQLException("executeBatch does not support mixing PreparedStatementSetter with other methods.");
        }
            ...
            for (int i = 0; i < batchArgs.length; i++) {
                ...
                if (i == 0) {
                    ...
                    if (!StringUtils.equals(prepareSql, sqlBuilder.getSqlString())) {
                        throw new RuntimeSQLException("executeBatch, each set of parameters must be able to derive the same SQL.");
                    } else {
                        prepareArgs[i] = sqlBuilder.getArgs();
                    }
                }
    ...
    @Override
    public int[] executeBatch(final String sql, final BatchPreparedStatementSetter pss) {
        if (logger.isDebugEnabled()) {
    ...
    /** 至返回结果集中的一条数据。 */
    private static <T> T requiredSingleResult(final Collection<T> results) {
        if (results == null || results.isEmpty()) {
            return null;
        }
        int size = results.size();
        if (size > 1) {
            throw new RuntimeSQLException("Incorrect record count: expected 1, actual " + size);
        }
