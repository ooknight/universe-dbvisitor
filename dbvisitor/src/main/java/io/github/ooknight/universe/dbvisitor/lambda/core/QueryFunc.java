...
public interface QueryFunc<R, T, P> extends BasicFunc<R>, BoundSqlBuilder {
    ...
    <V> V query(ResultSetExtractor<V> rse);

    /** 执行查询，并结果将被映射到一个列表(一个条目为每一行)的对象，列表中每一条记录都是<code>elementType</code>参数指定的类型对象。 */
    List<T> queryForList();

    /** 执行查询，并结果将结果映射到对象。 */
    <V> List<V> queryForList(Class<V> asType);

    /** 执行查询，并使用 RowMapper 处理结果集。 */
    <V> List<V> queryForList(RowMapper<V> rowMapper);
    ...
    /** 执行查询，并返回一个结果。 */
    <V> V queryForObject(Class<V> asType);

    /** 执行查询，并返回一个结果。 */
    <V> V queryForObject(RowMapper<V> rowMapper);
     ...
     */
    <K, V> Map<K, V> queryForPairs(P keyProperty, P valueProperty, Class<K> keyType, Class<V> valueType);

    /**
     * 执行查询，并将结果集的前两列（由 keyProperty 和 valueProperty 指定）映射为 Map&lt;K, V&gt; 返回。
     * <p>例: queryForPairs(“id”, “name”, Integer.class, String.class)</p>
     */
    <K, V> Map<K, V> queryForPairsByName(String keyPropName, String valuePropName, Class<K> keyType, Class<V> valueType);

    /** 生成 select count() 查询语句并查询总数。 */
    int queryForCount();

    /** 生成 select count() 查询语句并查询总数。 */
    long queryForLargeCount();
