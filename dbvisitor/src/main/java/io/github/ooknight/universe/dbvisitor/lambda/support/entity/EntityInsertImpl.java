 ...
 */
package io.github.ooknight.universe.dbvisitor.lambda.support.entity;
...
import io.github.ooknight.universe.dbvisitor.lambda.EntityInsert;
...
import io.github.ooknight.universe.dbvisitor.lambda.support.map.MapInsertImpl;
...
public class EntityInsertImpl<T> extends AbstractInsert<Insert<T>, T, SFunction<T>> implements EntityInsert<T> {
    ...
    @Override
    public Insert<T> applyEntity(T... entity) {
    ...
    protected SqlArg[][] buildInsertArgs(List<String> useColumns, boolean forExecute, Connection executeConn) {
    ...
    protected SqlArg[] buildArgsForMap(Map entity, List<ColumnMapping> mappings, boolean forExecute, Connection executeConn) {
    ...
    protected SqlArg[] buildArgsForEntity(Object entity, List<ColumnMapping> mappings, boolean forExecute, Connection executeConn) {
        SqlArg[] args = new SqlArg[mappings.size()];
        for (int j = 0; j < mappings.size(); j++) {
            ColumnMapping mapping = mappings.get(j);
    ...
    protected void processKeySeqHolderBefore(Connection conn, ColumnMapping mapping, Object entity, boolean isMap) {
